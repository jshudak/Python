...
def HumanPlayer(GameRecord):
    ...
    print("~~ Rock Paper Scissors, the Python Program! ~~")
    ...
    print('- Your choice of (r)ock, (p)aper, or (s)cissors')
...
def ComputerPlayer(GameRecord):
    ...
    '''
    rng = np.random.choice(['Rock', 'Paper', 'Scissors'], 1, p=getFrequency(GameRecord))[0]
    return rng
    
# %%
def getFrequency(GameRecord):
    '''
    Parameter: GameRecord (the record of both players' choices and outcomes)
    Return: A list of frequenies of the players' R, P, S choice
    '''
    R = P = S = 0
    i = 0
    
    while(i < len(GameRecord[0])):
        if(GameRecord[0][i] == 'Rock'):
            R += 1
        elif(GameRecord[0][i] == 'Paper'):
            P += 1
        else:
            S += 1
        i += 1
    ...
    R = R/(len(GameRecord[0]))
    P = P/(len(GameRecord[0]))
    S = S/(len(GameRecord[0]))
    
    # Order determined from your notes
